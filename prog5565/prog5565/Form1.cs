...
namespace prog5565
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM notes.users WHERE `user_name` = @UN AND `password` = @Pass", db.getConnection()); // команда на проверку в MySql
            ...
            if (table.Rows.Count > 0) // проверка сколько есть записей 
            {
                ...
                string id = "" ;
                MySqlCommand command1 = new MySqlCommand("SELECT id FROM notes.users WHERE `user_name` = @UN AND `password` = @Pass", db.getConnection()); // команда на поиск в MySql
